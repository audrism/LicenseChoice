...
1. **V2604 windows do not include author counts.** The c2dat-based author
   field needs a one-line addition to the stage-50 awk. The paper's Authors
   row in the V2604 BLvAL column carries over the V-mongo value from
   `out/prepost_vs_orig_design.csv` and is flagged with $\dagger$.
2. **Pt2Ptb has no V2604 build.** Upstream/downstream windows are
   computed at V and translated to V2604 IDs via `Pold2Pnew.modal.s`.
   Effects are stable across V and V2604 IDs (the V$\to$V2604 churn is
   <0.1%).
3. **Stars are not yet incorporated.** The GHArchive star map is being
   ingested separately; `compute_pre_window.py` already has the slot
   wired so it can be re-run when the events file lands.
