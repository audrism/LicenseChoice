...
While the World of Code (WoC) infrastructure offers a comprehensive
and powerful resource for analyzing open source software projects,
using its license map (P2L)~\cite{jahanshahi2024license} for our study comes with certain
limitations that must be acknowledged.  One key limitation is that
the P2L map relies on detecting license files committed to a
project's repository. While this approach does not assumes that
licenses are consistently and accurately recorded in dedicated
license.md files, it does not check the content of all
files~\cite{jahanshahi2024license}.  In practice, licenses could be
