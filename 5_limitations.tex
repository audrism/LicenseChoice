...
\paragraph{Regression-cohort selection.} The regression cohort is drawn
from the 22.3M projects that ever adopted a license (${\sim}17\%$ of the
131M ecosystem) and further restricted to those with exactly one license
type at first and last adoption, at least one year separating them, and
different first and last types---23{,}619 projects ($\sim$0.02\% of the
ecosystem). Projects that never license, license briefly and go
inactive, or churn among many license types are excluded by
construction. The reported associations therefore apply to projects that
both reach licensing and deliberately consolidate on a new license type;
they should not be read as describing the modal OSS project.
...
project counts in the year after first adoption; the same measure is
used in both the AFvAL and BLvAL designs). Platform-visibility signals
such as fork and star counts are not included; incorporating them is
left to future work.

\paragraph{Construct validity of activity metrics.} The seven outcomes
we model are commit-graph proxies rather than direct measures of project
health: a commit may be a large refactor or a typo fix, blob and file
growth mixes substantive additions with generated code, and active-months
treat a single maintenance commit the same as a productive burst. Our
conclusions describe associations with these proxies and should be
re-tested with richer measures (e.g., reviewed contributions, issue
turnaround) before being generalized.
